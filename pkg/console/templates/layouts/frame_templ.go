...
func Frame(pageTitle string) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "</div><div class=\"w-2/3\"><div class=\"relative\" x-data=\"searchBar()\" x-init=\"init()\" @navigate.window=\"window.location.href = $event.detail.path\"><div class=\"relative\"><input type=\"text\" placeholder=\"Search by block number, address, tx hash, track title, or username...\" class=\"w-full rounded-md border-2 border-gray-300 p-2 pr-24 bg-gray-50 dark:bg-gray-800 dark:text-gray-100\" x-model=\"query\" @input=\"handleInput\" @focus=\"showSuggestions = true\"><div class=\"absolute right-2 top-1/2 -translate-y-1/2 text-sm text-gray-500 dark:text-gray-400 bg-gradient-to-l from-gray-50 via-gray-50 to-transparent dark:from-gray-800 dark:via-gray-800 pl-12 rounded-r-md\" x-text=\"searchType\"></div></div><div class=\"absolute w-full mt-1 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 max-h-96 overflow-y-auto\" x-show=\"showSuggestions && suggestions.length > 0\" @click.away=\"showSuggestions = false\"><div class=\"py-1\"><template x-for=\"suggestion in suggestions\" :key=\"suggestion.id\"><div class=\"px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer\" :class=\"{ 'bg-gray-50 dark:bg-gray-700 font-medium': suggestion.isHeader }\" @click=\"selectSuggestion(suggestion); $dispatch('navigate', { path: getNavigationPath(suggestion) })\"><div class=\"text-sm truncate\" :class=\"suggestion.isHeader ? 'text-gray-500 dark:text-gray-400' : 'text-gray-900 dark:text-gray-100'\" x-text=\"suggestion.title\"></div><div x-show=\"!suggestion.isHeader\" class=\"text-xs text-gray-500 dark:text-gray-400 truncate\" x-text=\"suggestion.subtitle\"></div></div></template></div></div></div></div></div><div>")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
